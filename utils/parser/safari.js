...
const ReaderMinimumScore = 1600,
    ...
    NegativeRegEx = /advertisement|breadcrumb|combx|comment|contact|disqus|footer|link|meta|mod-conversations|promo|related|scroll|share|shoutbox|sidebar|social|sponsor|tags|toolbox|widget/i,
    ...
    LinkURLBadSearchParameterKeyMatchRegex = /author|comment|feed|id|related/i,
...
CandidateElement = function(e, t) {
    ...
    usableTextNodesInElement: function(t) {
        ...
        s(t);
        var o = t.childrenWithParallelStructure(),
            u = o.length;
        for (var a = 0; a < u; ++a) {
            var f = o[a];
            s(f)
    ...
    shouldPruneElement: function(t, n) {
        ...
        if (t.tagName === "OBJECT") {
            var b = t.querySelector("embed[src]"),
                w = b ? anchorForURL(b.src, this.contentDocument) : null;
        ...
        if (!this.commaCountIsLessThan(t, 10))
            return !1;
        var L = t.getElementsByTagName("p").length,
            A = t.getElementsByTagName("br").length,
            O = L + Math.floor(A / 2);
        if (T > O)
            return !0;
        ...
        if (m < 1 && M > s)
            return !0;
        if (t.tagName === "TABLE") {
            var _ = removeWhitespace(t.innerText).length,
                D = removeWhitespace(n.innerText).length;
            if (_ <= D * .5)
                return !0
        }
        return !1
    ...
    cleanArticleNode: function(t, n, r) {
        ...
        var a = [],
            ...
            C = this.dominantContentUnitSelector(t),
            k = T(),
            L = new Set;
        ...
        while (d) {
            ...
            if (!O && (M === "H1" || M === "H2")) {
                ...
                if (_ < HeaderMinimumDistanceFromArticleTop) {
                    var D = innerTextOrTextContent(d),
                        P = D.length * HeaderLevenshteinDistanceToLengthRatio;
            ...
            if (!O && M === "DIV" && m.parentNode) {
                var B = d.querySelectorAll("a, blockquote, dl, div, img, ol, p, pre, table, ul"),
                    j = l || H["float"] !== "none";
                if (!j && !B.length) {
                    var F = m.parentNode,
                        I = this.contentDocument.createElement("p");
                    while (m.firstChild) {
                        var q = m.firstChild;
                        I.appendChild(q)
                    }
            ...
            if (!O) {
                ...
                if (M === "TABLE")
                    ...
                    if (!l && !rt) {
                        var ot = cachedElementBoundingRect(d),
                            ut = ot.width,
                            at = ot.height;
        ...
        for (var W = 0; W < wt.length; ++W) {
            var Et = !1,
                St = wt[W];
            if (!Et) {
                var xt = St.querySelectorAll("a, span.converted-image-anchor"),
                    Tt = St.querySelectorAll("span.converted-anchor");
                Et = St.parentNode && Tt.length > xt.length
            }
            if (!Et) {
                var Nt = St.querySelectorAll("embed, object").length,
                    Ct = St.originalElement.querySelectorAll("embed, object").length;
                !Nt && Ct && (Et = !0)
            }
            if (!Et) {
                var kt = St.originalElement.getElementsByTagName("img"),
                    Lt = kt.length,
                    At = 0;
        ...
        if (w) {
            var _t = document.createElement("div"),
                Dt = w.originalElement.getBoundingClientRect(),
                Pt = Dt.height > 0 ? Dt.top * 100 / Dt.height : 0;
    ...
    articleTitle: function() {
        ...
        const t = 500,
            ...
            d = 1.5,
            v = /byline|author/i;
        var y = this.contentDocument.title,
            b = this.contentDocument.querySelector("head meta[property='og:title']"),
            w = b && b.attributes.length === 2 ? b.content : null,
            E = cachedElementBoundingRect(this.articleNode());
        this.extraArticleNode() && this.extraArticle.isPrepended && (E = cachedElementBoundingRect(this.extraArticleNode()));
        var S = E.left + E.width / 2,
            x = E.top,
            T = x;
        ...
        var C = "h1, h2, h3, h4, h5, .headline, .article_title, #hn-headline, .inside-head, .instapaper_title",
            k = this.article.element.tagName;
        if (k === "DL" || k === "DD")
            C += ", dt";
        var L = this.contentDocument.querySelectorAll(C);
        L = Array.prototype.slice.call(L, 0);
        var A = this.contentDocument.location,
            O = this.article.element.getElementsByTagName("a");
        ...
        var D, P = L.map(innerTextOrTextContent),
            H = L.length,
            B = 0,
            j = [],
            F = [];
        ...
        if (!D)
            for (var M = 0; M < H; ++M) {
                ...
                if (!isElementVisible(U))
                    continue;
                var z = cachedElementBoundingRect(U),
                    W = z.left + z.width / 2,
                    X = z.top + z.height / 2,
                    V = W - S,
                    $ = X - T,
                    ...
                    K = J,
                    Q = Math.sqrt(V * V + $ * $),
                    G = K ? t : Math.max(t - Q, 0),
                    I = P[M],
                    Y = U.getAttribute("property");
                if (Y) {
                ...
                if (y && stringsAreNearlyIdentical(I, y))
                    ...
                    continue;
                var tt = !1,
                    nt = nearestAncestorElementWithTagName(U, "A");
                nt || (nt = U.querySelector("a"));
                if (nt) {
                    var rt = nt.host === A.host,
                        it = nt.pathname === A.pathname;
                    if (rt && it)
                        G *= f;
                    else {
    ...
    siteName: function() {
        ...
        const t = [" - ", " \u2013 ", " \u2014 ", ": ", " | ", " \u00bb "],
            n = t.length;
        var r = this.contentDocument.title,
            i = this.contentDocument.location.host,
            s = this.articleTitle(),
            u, a;
        for (var f = 0; f < n; ++f) {
            var l = t[f],
...
function Parser(doc) {
    ...
    var ReaderArticleFinderJS = new ReaderArticleFinder(newDoc);
    var x = ReaderArticleFinderJS.isReaderModeAvailable();
